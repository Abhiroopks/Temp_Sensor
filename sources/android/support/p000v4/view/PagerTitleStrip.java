...
public class PagerTitleStrip extends ViewGroup {
    ...
    public void setNonPrimaryAlpha(@FloatRange(from = 0.0d, mo117to = 1.0d) float alpha) {
    ...
    public void updateTextPositions(int position, float positionOffset, boolean force) {
        ...
        int maxBaseline = Math.max(Math.max(alignedPrevHeight, alignedCurrHeight), nextTopOffset + this.mNextText.getMeasuredHeight());
        int i11 = alignedPrevHeight;
        int alignedPrevHeight2 = this.mGravity & 112;
        int i12 = alignedCurrHeight;
        if (alignedPrevHeight2 == 16) {
            int centeredTop = (((stripHeight2 - paddingTop) - paddingBottom) - maxBaseline) / 2;
            ...
            int bottomGravTop = (stripHeight2 - paddingBottom) - maxBaseline;
