...
public abstract class BaseSeries<E extends DataPointInterface> implements Series<E> {
    ...
    public Iterator<E> getValues(double from, double until) {
        ...
        return new Iterator<E>() {
            ...
            E nextValue = null;
            Iterator<E> org = BaseSeries.this.mData.iterator();
            boolean plusOne = true;

            {
                boolean found = false;
                E prevValue = this.org.hasNext() ? (DataPointInterface) this.org.next() : null;
                if (prevValue != null) {
                    if (prevValue.getX() < d) {
                        while (true) {
                            if (!this.org.hasNext()) {
    ...
    public void onTap(float x, float y) {
        E p;
        if (this.mOnDataPointTapListener != null && (p = findDataPoint(x, y)) != null) {
