...
public class CursorMode {
    ...
    public void resetStyles() {
        ...
        int color1 = ViewCompat.MEASURED_STATE_MASK;
        try {
            TypedArray array = this.mGraphView.getContext().obtainStyledAttributes(typedValue.data, new int[]{16842806});
            int color12 = array.getColor(0, ViewCompat.MEASURED_STATE_MASK);
            array.recycle();
            color1 = color12;
        } catch (Exception e) {
        }
    ...
    public void drawLegend(Canvas canvas) {
        ...
        if (legendWidth == 0 && (legendWidth = this.cachedLegendWidth) == 0) {
        ...
        float legendPosY = (this.mPosY - legendHeight) - (4.5f * this.mStyles.textSize);
        if (legendPosY < 0.0f) {
            legendPosY = 0.0f;
        }
        ...
        while (it.hasNext()) {
            Map.Entry<BaseSeries, DataPointInterface> entry2 = it.next();
            ...
            canvas2.drawText(getTextForSeries(entry2.getKey(), entry2.getValue()), ((float) this.mStyles.padding) + lLeft + ((float) shapeSize) + ((float) this.mStyles.spacing), ((float) (this.mStyles.padding / 2)) + lTop + this.mStyles.textSize + (((float) i) * (this.mStyles.textSize + ((float) this.mStyles.spacing))), this.mTextPaint);
            i++;
            legendWidth = legendWidth;
            legendPosX = legendPosX;
            legendHeight = legendHeight;
            legendPosY = legendPosY;
            it = it;
            lRight = lRight;
        }
        float f = legendPosX;
    ...
    private void findCurrentDataPoint() {
        DataPointInterface p;
        double selX = 0.0d;
        this.mCurrentSelection.clear();
        for (Series series : this.mGraphView.getSeries()) {
            if ((series instanceof BaseSeries) && (p = ((BaseSeries) series).findDataPointAtX(this.mPosX)) != null) {
                selX = p.getX();
